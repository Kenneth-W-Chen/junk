    ...
    def read_errors(self):
        ...
        for i in range(len(self.address)):
            ...
            if err_int != 0:
                ...
                if (has_error):
                    self.log.error(
                        f"Motor controller {self.address[i]} reported error code {err[i]} (hex: {hex(err_int)}),{err_string}")
                else:
                    self.log.warn(
                        f"Motor controller {self.address[i]} reported warning code {err[i]} (hex: {hex(err_int)}), {err_string}")
